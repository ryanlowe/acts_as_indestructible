...
class OptionsTest < ActiveSupport::TestCase
  #DEFAULT_OPTIONS = Hash.new({ :conditions => "deleted_at IS NULL" }
  ...
  def test_options_excluding_deleted_no_options
    ...
    assert_equal expected_default, Post.options_excluding_deleted(Hash.new)
    assert_equal expected_default, Post.options_excluding_deleted(nil)
  end
  ...
  def test_options_excluding_deleted_no_conditions
    ...
    assert_equal expected_with_order, Post.options_excluding_deleted({ :order => "id DESC" })
    
    expected_with_group = expected_default.clone
    expected_with_group[:group] = "title"
    assert_equal expected_with_group, Post.options_excluding_deleted({ :group => "title" })
  end
  
  def test_options_excluding_deleted_with_string_conditions
    expected = { :conditions => "deleted_at IS NULL AND 1=1" }
    
    assert_equal expected, Post.options_excluding_deleted({ :conditions => "1=1" })
  end
  
  def test_options_excluding_deleted_with_array_conditions
    expected = { :conditions => ["deleted_at IS NULL AND 1=?","fried chicken"] }
    
    assert_equal expected, Post.options_excluding_deleted({ :conditions => ["1=?","fried chicken"] })
  end
  ...
  def test_options_excluding_deleted_with_string_conditions_and_deleted_at
    ...
    assert_equal expected, Post.options_excluding_deleted({ :conditions => "deleted_at IS NULL AND 1=1" })
  end
  ...
  def test_options_excluding_deleted_with_array_conditions_and_deleted_at
    ...
    assert_equal expected, Post.options_excluding_deleted({ :conditions => ["deleted_at IS NULL AND 1=?","fried chicken"] })
  end
